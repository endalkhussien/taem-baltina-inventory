...
export function useIngredients() {
  ...
  return { ...query, createIngredient: (d: any) => create.mutateAsync(d), updateIngredient: (id: number, d: any) => update.mutateAsync({ id, data: d }), deleteIngredient: async (id: number) => { await fetch(`/api/ingredients/${id}`, { method: 'DELETE' }); qc.invalidateQueries({ queryKey: ['ingredients'] }) } }
}
...
export function useSales() {
  ...
  const create = useMutation({ mutationFn: async (data: any) => { const res = await fetch('/api/sales', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); if (!res.ok) throw new Error('Create failed'); return res.json() }, onSuccess: () => qc.invalidateQueries({ queryKey: ['sales'] }) })
  return { ...query, createSale: (d: any) => create.mutateAsync(d), deleteSale: async (id: number) => { await fetch(`/api/sales/${id}`, { method: 'DELETE' }); qc.invalidateQueries({ queryKey: ['sales'] }) } }
}
